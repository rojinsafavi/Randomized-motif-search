...
class SoftDecoding(object):
    ...
    def forwardBackward(self):
        ...
        for characterIndex in range(len(self.observation)-2, -1,-1): # for each position in the observed, first I calculated the max probability of the path that ends in that position, then I multiply that by the emission
            backwardProb.append({}) #for each position append a new dict to the ForwardProb
            for state in self.states: # A,B,C ...
                SUM = 0
                for prev_state in self.states:
    ...
    def softDecoding(self):
        ...
        for i, j in enumerate(forward):
            ...
            for state in self.states:
                #A = '%.4g'%(j[state]['currentProb']*h[state]['currentProb']/total)
                #A = round(A,4)
