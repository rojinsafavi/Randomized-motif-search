...
class ViterbiPath(object):
    ...
    def viterbiPath(self):
        ...
        print('observedChar', self.observedChar)
        ...
        returns:
            A path of states'''
        probPrev = [{state : {"currentProb" : (1.0 / len(self.states)) * self.emission[self.states[state]][self.observedChar[self.observation[0]]],"prevState" : None} for state in self.states}] # starting a dict with prev state of None and current robability of neing in state A or B as currentProb
        for characterIndex in range(1, len(self.observation)): # for each position in the observed, first I calculated the max probability of the path that ends in that position, then I multiply that by the emission
            ...
            for state in self.states: # A,B,C ...
                ...
                for prev_state in self.states:
                    prob, prev_st = (probPrev[characterIndex-1][prev_state]["currentProb"] * self.transition[self.states[prev_state]][self.states[state]] * self.emission[self.states[state]][self.observedChar[self.observation[characterIndex]]], prev_state)
                    if prob >= maxProb:
                        maxProb = prob
                        prevState = prev_st
...
def main():
    ...
    V = ViterbiPath(observation = observation, transition =transition, emission = emission, states = states  , observedChar =  observedChar)
