            ...
            for k in prevNode: # itterate over them, and pick the one that satisfies nodeScore - edgeScore = previous Node Score
                if k in scoreDic.keys():
                    if (endScore - self.setDic[(k, end)]) == scoreDic[k][0] :
                        p.append(k)
                        score = scoreDic[k][0]
                        ##Ris: You need to break here  other wise you get the bug on branches
            end = p[-1]
        return final, p[::-1]
...
def main():
    ...
    ##Would be cool to wrap this in a method would look like
    ##setDic = parseDag(sys.stdin)
    ...
    if source == sink :
        ##Ris: Nice special case but this can and should be handled by the algorithm
